...
public class PWMUtils {
	...
	public static void main (String [] args) throws IllegalArgumentException, Exception {
		...
		if("scan".equalsIgnoreCase(argMap.getTask())) {
			...
			GenericOrganism go = new GenericOrganism(new File(seqDir));
			...
			while(chrIt.hasNext()) {
				...
				Chromosome c = go.getChromosome(chr.replace("chr", ""));
				System.err.print("Scanning regions on " + chr + " memory " + (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory())/1000);
				c.loadSequence();
			...
			List<Chromosome> nonRandomChromsomes = go.getAllNonRandomChromosomes();
			
			for(Chromosome c : nonRandomChromsomes) {
				List<SequenceRegion> regions = c.chunk(100000, 0);
	...
	private static double[] computeSequenceBGSequences(GenericOrganism org, Map<String, List<? extends GenomicAnnotation>> regionChrMap) throws IOException {
		...
		while(chrIt.hasNext()) {
			String chr = chrIt.next();
			Chromosome c = org.getChromosome(chr.replace("chr", ""));
			c.loadSequence();
			Sequence chrSeq = c.getSequence();
	...
	public static double[] computeSequenceBGSequences(GenericOrganism org, AnnotationReader<? extends GenomicAnnotation> reader) throws IOException {
		double [] bgFreqs = new double [4];
		int totalBases = 0;
		logger.debug("Computing sequences background nucleotide dsitribution");
