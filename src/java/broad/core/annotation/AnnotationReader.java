...
public abstract class AnnotationReader <T extends GenomicAnnotation> implements AlignmentCollection {
	...
	*/
	...
	public List<T> getAnnotations(int setNumber, String name ) {
		AnnotationSet<T> set = annotationSetMap.get(setNumber);
		return set.getAnnotations(name);
	}
	...
	public List<T> getAnnotations(String name ) {
		return getAnnotations(0, name);
	}
	...
	public static class AnnotationSet <T extends LightweightGenomicAnnotation>{
		...
		Map<String, List<T>> nameAnnotationMap;
		...
		public AnnotationSet() {
			...
			nameAnnotationMap = new HashMap<String, List<T>>();
		}
		...
		public void addAnnotation(T annotation) {
			...
			if(annotation.getStart() > annotation.getEnd()) {
				...
				if(nameAnnotationMap.containsKey(name)) {
					nameAnnotationMap.get(name).add(annotation);
				} else {
					List<T> nameList = new ArrayList<T>();
					nameList.add(annotation);
		...
		public T getAnnotation(String name) {
			T rtrn = null;
			if(nameAnnotationMap.containsKey(name)) {
				rtrn = nameAnnotationMap.get(name).get(0); //RETURN THE FIRST
			}
			return rtrn;
		}
		/**
		 * Sometimes you get more than one annotation with the same name!
		 * @param name
		 * @return
		 */
		public List<T> getAnnotations(String name) {
