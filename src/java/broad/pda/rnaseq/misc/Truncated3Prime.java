...
public class Truncated3Prime {

	public Truncated3Prime(Collection<RefSeqGene> genes, Collection<RefSeqGene> predictions, String save) throws IOException{
		...
		for(RefSeqGene gene: genes){
			...
			Collection<RefSeqGene> truncated=getTruncated3Prime(overlappers, gene, exonTree);
	...
	private Collection<RefSeqGene> getTruncated3Prime(Iterator<Node<RefSeqGene>> overlappers, RefSeqGene gene, Map<String, IntervalTree<Alignments>> exonTree) {
		...
		while(overlappers.hasNext()){
			...
			if(prediction.getOrientation().equalsIgnoreCase(gene.getOrientation())){
				...
				if(!prediction.get3PrimeExon().overlapsAtAll(gene.get3PrimeExon())){
					int compareTo=prediction.get3PrimeExon().compareTo(gene.get3PrimeExon());
					...
					if(truncated){
						Alignments lastExon=prediction.get3PrimeExon();
						...
						if(!overlapsKnown){
							boolean is5PrimeComplete=is5PrimeComplete(prediction, gene);
							if(is5PrimeComplete){rtrn.add(prediction); System.out.println(prediction.get3PrimeExon());}
						}
					}
				}
			}
	...
	public static void main(String[] args) throws IOException{
		if(args.length>2){
			...
			String save=args[2];
			new Truncated3Prime(genes, predictions, save);
		}
