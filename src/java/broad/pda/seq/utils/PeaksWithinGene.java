...
public class PeaksWithinGene {
	double alpha=0.05;
	//int[] windowSizes={25,50,100,150,200,300,400,500,600,700,800,900,1000,1500,2000,2500,3000};
	int extensionLength=50000;
	int[] windowSizes={100,300,1000};
	
	public PeaksWithinGene(AlignmentDataModel alignmentModel, Map<String, RefSeqGene> genes, String save, int extensionFactor, Collection<String> queryGenes) throws IOException{
	...
	private Map<String, Collection<Alignments>> getSegments(AlignmentDataModel alignmentModel, ContinuousDataAlignmentModel dataModel, int[] windowSizes, Map<String, RefSeqGene> genes, Map<String, IntervalTree<RefSeqGene>> trees, Collection<String> queryGenes) throws IOException {
		...
		for(String name: queryGenes){
			RefSeqGene gene=genes.get(name);
			//System.err.println(name+" "+gene);
			Alignments extended=getExtension(gene, trees);
			System.err.println(gene.getAlignment().toUCSC()+" "+extended.toUCSC());
		...
		for(String name: genes.keySet()){
			RefSeqGene gene=genes.get(name);
			chromosomes.add(gene.getChr());
		}
		
		Collection<Alignments> rtrn=new TreeSet<Alignments>();
		for(String chr: chromosomes){
			System.err.println(chr);
			rtrn.addAll(dataModel.scan(windowSizes, alpha, true, chr));
		}
		return rtrn;*/
	}
	...
	private Collection<Alignments> refToAlign(Collection<Path> genes) {
		...
		for(Path gene: genes){
			rtrn.add(gene.toGene().getAlignment());
		}
	...
	public static void main(String[] args)throws IOException{
		if(args.length>4){
			...
			new PeaksWithinGene(alignments, genes, save, extensionFactor, queryGenes);
		}
