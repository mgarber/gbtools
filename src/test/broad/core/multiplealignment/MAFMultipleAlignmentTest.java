...
public class MAFMultipleAlignmentTest extends TestCase {
	...
	protected void setUp() throws Exception{
		...
		URI testUri = new URI(cleanDropboxNames(mafTestURL.getPath()));
		String idxFileName = testUri.getRawPath() + ".index";
		System.out.println("test.maf index file: " + idxFileName);
		File idxFile = new File(idxFileName);
		if(!idxFile.exists()) {
			test = new MAFAlignment();
			test.createIndex(testUri.getRawPath());
			test.writeIndex(idxFileName);
			test.cleanReferencesFromIndex();
		}
		test = new MAFAlignment(idxFileName);
	}
	
	/**
	 * This is a hack to make resources within the dropbox directories which contain spaces and parenthesis work.
	 * To make this tests works you need to create a soft or hard link that is named Dropbox only. This method should 
	 * be removed once a real solution is found
	 * @throws Exception
	 */
	protected String cleanDropboxNames (String path) {
		return path.replace("%20(Personal)", "");
	}
	
	public void testConcatenation2() throws Exception {
		URL mafTestURL = getClass().getResource("test2.maf");	
		MAFIO mafio = new MAFIO();
		MAFAlignment test2 = mafio.load(cleanDropboxNames(mafTestURL.getFile()));
	...
	public void testLoadOnlyRequestedSeqs() throws Exception {
		...
		test = mafio.load(cleanDropboxNames(mafTestURL.getFile()), seqsToLoad);
	...
	public void testCountAligningSequences() throws Exception{
		URL mafTestURL = getClass().getResource("test.maf");		
		test.load(cleanDropboxNames(mafTestURL.getFile()));
	...
	public void testCompressSubalignment() throws Exception {
		...
		test.load(cleanDropboxNames(mafTestURL.getFile()), 119880 + 519, 120475 + 5);
		test.compress();
	...
	public void testClosestOffset() throws Exception {
		assertEquals(117271,test.getClosestOffset("chr5",111294));
		assertEquals(117757,test.getClosestOffset("chr5",111295));
		assertEquals(169333,test.getClosestOffset("chr5",125817));
	}
	
	public void testLoadSubalignment() throws IOException, ParseException {
		URL mafTestURL = getClass().getResource("test.maf");		
		test.load(cleanDropboxNames(mafTestURL.getFile()),111323,111333);
		AlignedSequence seq = test.getAlignedSequence("hg18");
		assertEquals("TGAACAAGAT", seq.getSequenceBases());
		...
		//MultipleAlignment ma = test.getSubAlignment(111323, 111333, false);
		test.load(cleanDropboxNames(mafTestURL.getFile()), 111486, 111494);
		...
		test.load(cleanDropboxNames(mafTestURL.getFile()), 119880 + 520, 119880 + 530);
		BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
		test.write(bw);
		bw.flush();
		seq = test.getAlignedSequence("hg18");
		...
		test.load(cleanDropboxNames(mafTestURL.getFile()), 71257 + 330, 71257 + 331);
		seq = test.getAlignedSequence("hg18");
	...
	public void testCreateIndex() throws java.io.IOException, ParseException{
		URL mafTestURL = getClass().getResource("test.maf");
		System.out.print("opening " +cleanDropboxNames( mafTestURL.getFile()));
		MAFAlignment testAln = new MAFAlignment();
		System.out.print(" ... creating index ");
		testAln.createIndex(cleanDropboxNames(mafTestURL.getFile()));
		testAln.cleanReferencesFromIndex();
		System.out.println(" .Done");
		IntervalTree<Long> index = testAln.getIndex("chr5");
		RandomAccessFile raf = new RandomAccessFile(cleanDropboxNames(mafTestURL.getFile()), "r");
		try {
